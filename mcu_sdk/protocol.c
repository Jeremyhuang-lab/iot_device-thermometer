...
//    #error "请将MCU串口发送函数填入该函数,并删除该行"
    ...
    //Example:
    extern void Uart1_PutChar(unsigned char bdat);
    Uart1_PutChar(value);	                                //串口发送函数
}
...
//    mcu_dp_value_update(DPID_TEMP_CURRENT,200); //VALUE型数据上报;
//    mcu_dp_value_update(DPID_HUMIDITY_VALUE,200); //VALUE型数据上报;
    mcu_dp_enum_update(DPID_BATTERY_STATE,1); //枚举型数据上报;
    mcu_dp_value_update(DPID_BATTERY_PERCENTAGE,20); //VALUE型数据上报;
    mcu_dp_bool_update(DPID_TAMPER_ALARM,0); //BOOL型数据上报;
    mcu_dp_value_update(DPID_TEMP_SAMPLING,1); //VALUE型数据上报;
    mcu_dp_value_update(DPID_HUMIDITY_SAMPLING,1); //VALUE型数据上报;
    mcu_dp_bool_update(DPID_CHARGE_STATE,0); //BOOL型数据上报;
    mcu_dp_enum_update(DPID_TEMP_UNIT_CONVERT,0); //枚举型数据上报;
    mcu_dp_value_update(DPID_MAXTEMP_SET,250); //VALUE型数据上报;
    mcu_dp_value_update(DPID_MINITEMP_SET,250); //VALUE型数据上报;
    mcu_dp_value_update(DPID_MAXHUM_SET,250); //VALUE型数据上报;
    mcu_dp_value_update(DPID_MINIHUM_SET,250); //VALUE型数据上报;
    mcu_dp_enum_update(DPID_TEMP_ALARM,10); //枚举型数据上报;
    mcu_dp_enum_update(DPID_HUM_ALARM,10); //枚举型数据上报;
    mcu_dp_value_update(DPID_BRIGHT_VALUE,0); //VALUE型数据上报;
    mcu_dp_value_update(DPID_TEMP_PERIODIC_REPORT,2); //VALUE型数据上报;
    mcu_dp_value_update(DPID_HUM_PERIODIC_REPORT,2); //VALUE型数据上报;
    mcu_dp_value_update(DPID_TEMP_SENSITIVITY,2); //VALUE型数据上报;
    mcu_dp_value_update(DPID_HUM_SENSITIVITY,2); //VALUE型数据上报;
